...
def load_dataset(ds_name):
    ...
    elif ds_name == "mnist-circuit":
        transform=v2.Compose([
            v2.ToTensor(),
            v2.Normalize((0.1307,), (0.3081,))
        ])

        trainset = torchvision.datasets.MNIST(root="./data",train=True, transform=transform, download=True, target_transform=lambda x: 1 if x in (0,4,6,8,9) else 0)
        testset  = torchvision.datasets.MNIST(root="./data",train=False, transform=transform, download=True, target_transform=lambda x: 1 if x in (0,4,6,8,9) else 0)
    else:
